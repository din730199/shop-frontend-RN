...
export default class ProductMain extends Component {
  ...
  getListProductType = async ()=>{
    ...
    renderItems(product) {
      return (
        <TouchableOpacity style={styles.productContainer} onPress={() => this.props.navigation.navigate("ListProduct",{idType: product._id})}>
            <View style={{ height:produtWidth/4, justifyContent: 'center' }}>
                <Text style={styles.textStyle} >{product.name.toUpperCase()}</Text>
            </View>
            <View style={{ flex: 4, justifyContent: 'flex-end' }}>
                <Image source={{ uri: `https://mainf-app.herokuapp.com${product.image}` }} style={styles.imageStyle} />
            </View>
        </TouchableOpacity>
      );
    }
